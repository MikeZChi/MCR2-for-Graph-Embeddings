...
class MaximalCodingRateReduction(torch.nn.Module):
    ...
    def compute_discrimn_loss_combined(self, W1,W2):
        ...
        if torch.isnan(logdet):
            logdet = torch.logdet(I)
        return logdet / 2.

    def compute_compress_loss_separated(self, W1,W2):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        I1 = torch.eye(p1)
        I1 = I1.to(device)
        scalar1 = p1 / (m1 * self.eps)
        logdet1 = torch.logdet(I1 + self.gam1 * scalar1 * W1.matmul(W1.T))
        if torch.isnan(logdet1):
            logdet1 = torch.logdet(I1)

        p2, m2 = W2.shape
        I2 = torch.eye(p2)
        I2 = I2.to(device)
        scalar2 = p2 / (m2 * self.eps)
        logdet2 = torch.logdet(I2 + self.gam1 * scalar2 * W2.matmul(W2.T))
        compress_loss = logdet1 + logdet2
        if torch.isnan(logdet2):
            logdet2 = torch.logdet(I2)
    ...
    def forward(self, X1, X2):
        ...
        discrimn_loss_empi = self.compute_discrimn_loss_combined(W1,W2)
        compress_loss_empi = self.compute_compress_loss_separated(W1,W2)
...
if __name__ == '__main__': 
    datasets_names = ['MUTAG', 'PTC_MR', 'PROTEINS', 'NCI1', 'IMDB-BINARY', 'IMDB-MULTI', 'REDDIT-BINARY','REDDIT-MULTI-5K']
    for i in range(len(datasets_names)):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for data in dataloader:
                ...
                batch_labels = list(set(batch_idx))
                batch_idx = torch.from_numpy(np.array(batch_idx)).to(torch.long).to(device)
                # Set embeddings in subsets
                loss_cum = 0
                for j in range(len(batch_labels)-1):
                    sub_loss_cum = 0
                    max_idx = max(batch_labels)
                    next_idx = batch_labels[j+1]
                    target_label_1 = batch_labels[j]
                    mask_1 = (batch_idx == target_label_1)
                    subset_embeddings_1 = embeddings[mask_1]
                    for jj in range(next_idx,max_idx+1):
                        target_label_2 = batch_labels[jj]
                        mask_2 = (batch_idx == target_label_2)
                        subset_embeddings_2 = embeddings[mask_2] 
                        sub_loss = criterion(subset_embeddings_1,subset_embeddings_2)
                        sub_loss_cum += sub_loss
                    loss_cum += sub_loss_cum
                loss_all += loss_cum.item()
                loss_cum.backward()
                optimizer.step()
